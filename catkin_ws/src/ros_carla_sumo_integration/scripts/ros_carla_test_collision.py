...
def collision_handler(event):
    ...
    print("actor {} is collided".format(actor_we_collide_against))
    # Stop the vehicle
    traffic_manager.vehicle_percentage_speed_difference(actor_we_collide_against, 0)
    # Wait for the vehicle to stop
    while actor_we_collide_against.get_velocity() != carla.Vector3D(0, 0, 0):
        continue
    # Delete the vehicle
    actor_we_collide_against.destroy()
...
class VehicleState():
    def __init__(self):        
        ...
        self.current_state = {'t':0, 'x':120, 'y':270, 'psi':12,'lat':0, 'lon':0, 'alt':1, 'v_lon': 10}
        self.state_traj = []
        self.check = 0
        # rospy.spin()
...
def main():
    ...
    try:
        ...
        traffic_manager.global_percentage_speed_difference(90.0) # speed limit can be exceeded by -30% (percent is applied in a negative way)
        ...
        while not rospy.is_shutdown():
            ...
            # Detector location same as ego vehicle
            collision_detector_location = carla.Location(x=xego_cur, y=yego_cur)
            collision_detector_transform = carla.Transform(collision_detector_location)
            collision_detector = world.spawn_actor(collision_detector_bp, collision_detector_transform, attach_to=vehicle)

            # Register the collision detector callback
            collision_detector.listen(lambda event: collision_handler(event))
            ...
            for actor_id in npc_list:                  
                ...
                collision_dist = 5.0
                dist = vehicle.get_location().distance(actor_temp.get_location())
            ...
            if synchronous_master:
                ...
                if i%1 == 0:                  
                    ...
                    trans.location.x = 136.468244438
                    trans.location.y = -118.663684215
                    trans.rotation.pitch = 0
                    trans.rotation.roll = 0

                    # Teleport the vehicle in CARLA,
                    # import pdb; pdb.set_trace()
                    print("vehicle loc:{}".format(vehicle.get_location()))
                    print("tran loc: {}".format(trans.location))
